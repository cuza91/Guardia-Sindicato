    ...
    document.querySelectorAll('.delete-worker').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (opcion === "1") {
                ...
                workers = workers.filter(w => w.id !== id);
                alert(`Trabajador "${worker.name}" eliminado. Se conservaron ${guardCount} guardias (aparecerán como "❌ Eliminado" en la tabla).`);
            }
            else if (opcion === "3") {
                if (workers.length === 1) {
                    alert("No hay otros trabajadores para reasignar las guardias. Elimina el trabajador sin reasignar.");
                    return;
                }
                ...
                saveData();
                refreshUI();
                alert(`Trabajador "${worker.name}" eliminado. Sus ${guardCount} guardias fueron reasignadas a "${newWorker.name}".`);
                return;
            }
            saveData();
            refreshUI();
...
function renderGuardDaysList() {
    ...
    if (guardDays.length === 0) {
        container.innerHTML = '<p>No hay días definidos. Añade fechas específicas para las guardias.</p>';
        return;
    }
    ...
    document.querySelectorAll('.remove-day').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (confirm(`¿Eliminar el día ${date} de la lista?`)) {
                ...
                saveData();
                renderGuardDaysList();
                if (confirm('¿Deseas eliminar también las guardias asignadas a ese día?')) {
...
function generateGuardsFromDays() {
    ...
    saveData();
    refreshUI();
    alert(`Guardias generadas para ${newGuards.length} días definidos. Se conservaron ${otherGuards.length} guardias de otras fechas. Próximo trabajador índice: ${lastWorkerIndexForDays}`);
}
...
function regenerateGuards() {
    ...
    combined.sort((a,b)=>a.date.localeCompare(b.date));
    combined.forEach((g, idx) => { g.id = idx+1; });
    guards = combined;
    saveData();
    refreshUI();
    alert(`Guardias regeneradas para ${currentYear}. Días laborables: ${newLaborableGuards.length}`);
}

// ---------- GUARDIAS MANUALES ----------
function openManualModal() {
    if (workers.length === 0) { alert('Primero añade trabajadores.'); return; }
...
function saveManualGuard() {
    ...
    if (!fechaStr) { alert('Selecciona una fecha.'); return; }
...
function updateFilterYearSelect() {
    ...
    guards.forEach(g => { const y = parseInt(g.date.split('-')[0]); if(!isNaN(y)) yearsSet.add(y); });
...
function renderGuardsTablePage() {
    ...
    for (const guard of pageGuards) {
        ...
        chk.type='checkbox'; chk.checked=guard.completed;
        chk.addEventListener('change',()=>{ guard.completed=chk.checked; saveData(); renderSummary(); });
        chkCell.appendChild(chk);
        const actionsCell = row.insertCell(3);
        const editBtn = document.createElement('button');
        editBtn.textContent='✏️ Editar'; editBtn.className='btn-edit';
        editBtn.style.marginRight='5px';
        editBtn.addEventListener('click',()=>openEditModal(guard));
        actionsCell.appendChild(editBtn);
        const delBtn = document.createElement('button');
        delBtn.textContent='🗑️ Eliminar'; delBtn.className='btn-edit';
        delBtn.style.backgroundColor='#f8d7da'; delBtn.style.color='#721c24';
...
function renderPaginationControls(totalPages) {
    ...
    container.querySelectorAll('button[data-page]').forEach(btn=>{
        btn.addEventListener('click',()=>{
            ...
            if(!isNaN(page) && page>=1 && page<=totalPages) { currentPage=page; renderGuardsTablePage(); }
        });
    });
}
...
function saveEditGuard() {
    ...
    if(!newDate) { alert('Selecciona fecha'); return; }
...
function renderCalendar() {
    ...
    for(let w of weekdays) gridHtml += `<div class="calendar-day-header">${w}</div>`;
    for(let i=0; i<startDayOfWeek; i++) gridHtml += `<div class="calendar-day-cell calendar-empty"></div>`;
    ...
    document.querySelectorAll('.calendar-day-cell[data-date]').forEach(cell => {
        cell.addEventListener('click', () => {
            ...
            else alert(`No hay guardia asignada para ${formatDate(date)}. Puedes añadirla manualmente.`);
        });
    });
}
...
function renderSummary() {
    ...
    if(workers.length===0) { container.innerHTML='<p>No hay trabajadores.</p>'; return; }
    if(guards.length===0) { container.innerHTML='<p>No hay guardias generadas.</p>'; return; }
    let filteredWorkers = [...workers];
    if(filterValue === 'noGuards') {
        filteredWorkers = workers.filter(w => guards.filter(g=>g.workerId===w.id).length===0);
    ...
    if(filteredWorkers.length===0) { container.innerHTML='<p>No hay trabajadores que coincidan con el filtro.</p>'; return; }
    const totalGuards = guards.length;
    const completedGuards = guards.filter(g=>g.completed).length;
    let html = `<div class="stat-card" style="background:#e6f7f0;"><h3>📊 Total general</h3><p>${completedGuards}/${totalGuards}</p><p>${Math.round(completedGuards/totalGuards*100)}% completado</p></div>`;
...
function exportToCSV() {
    if(guards.length===0) { alert('No hay guardias para exportar.'); return; }
...
function loadExampleWorkers() {
    if(workers.length>0 && !confirm('¿Sobrescribir los trabajadores actuales?')) return;
    workers = [
...
function clearAllData() {
    if(!confirm('⚠️ Eliminará TODOS los trabajadores, guardias y días definidos. ¿Continuar?')) return;
    workers = [];
    guards = [];
    guardDays = [];
    lastWorkerIndexForDays = 0;
...
function bindEvents() {
    ...
    document.getElementById('loadExampleBtn')?.addEventListener('click', loadExampleWorkers);
...
function init() {
    ...
    if (guards.length === 0 && workers.length > 0) {
        console.log('Usa "Generar Guardias Automáticamente" o define días y genera.');
    }
}
init();