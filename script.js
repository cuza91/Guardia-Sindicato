...
function renderWorkerSelectForManual(filterText = "") {
  ...
  // Mantener la opción "Sin asignar" al inicio
  ...
  if (!filtered.length) {
    ...
    workerSelect.selectedIndex = 0; // seleccionar "Sin asignar" por defecto
  }
}
...
function saveManualGuard() {
  ...
  // No es obligatorio tener trabajador
  ...
  sorted.sort((a, b) => {
    ...
    switch (currentSortColumn) {
      case "date":
        ...
        break;
      case "worker":
        // Si workerId es null, considerar como cadena vacía para que vayan al final
...
function applyFiltersAndRenderTable() {
  ...
  // Filtro trabajador: "none" para sin asignar
...
function openEditModal(guard) {
  ...
  // Llenar select de trabajadores con opción vacía
  workerSelect.innerHTML = '<option value="">— Sin asignar —</option>';
  workers.forEach((w) => {
    const opt = document.createElement("option");
    opt.value = w.id;
  ...
  // Si guard.workerId es null, seleccionar la opción vacía
  if (guard.workerId == null) {
    workerSelect.value = "";
  } else {
    workerSelect.value = guard.workerId;
  }
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (guard)
      grid += `<div class="calendar-day-guard ${completed ? "completed" : ""}">${escapeHtml(workerName)}<br>${completed ? "✅" : "⏳"}</div>`;
    grid += `</div>`;
  }
  ...
  document.querySelectorAll(".calendar-day-cell[data-date]").forEach((cell) => {
    cell.addEventListener("click", () => {
      ...
      if (guard) openEditModal(guard);
        ...
        alert(
          `No hay guardia para ${formatDate(date)}. Puedes añadirla manualmente.`,
        );
    });
  });
}
...
function renderSummary() {
  ...
  let html = `<div class="stat-card" style="background:#e6f7f0;">
                ...
                <div class="progress" style="font-size:0.85rem;">${Math.round((completedTotal / total) * 100)}% completado</div>
  ...
  // Mostrar tarjetas por trabajador (solo si hay trabajadores)
  if (workers.length > 0) {
