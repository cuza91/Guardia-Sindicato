        ...
        btn.addEventListener("click", async () => {
            ...
            try {
                if (opcion === "1") {
                    ...
                    await fetch(API_URL + `guards.php?worker_id=${id}`, { method: 'DELETE' });
                    await fetch(API_URL + `workers.php?id=${id}`, { method: 'DELETE' });
                    workers = workers.filter((w) => w.id !== id);
                    guards = guards.filter((g) => g.worker_id !== id);
                    alert(`Trabajador "${worker.nombre}" eliminado junto con sus ${guardCount} guardias.`);
                } else if (opcion === "2") {
                    ...
                    alert(`Trabajador "${worker.nombre}" eliminado. Se conservaron ${guardCount} guardias.`);
                } else if (opcion === "3") {
                    if (workers.length === 1) {
                    ...
                    await saveData();
                    refreshUI();
                    alert(`Trabajador "${worker.nombre}" eliminado. ${guardCount} guardias reasignadas a "${newWorker.nombre}".`);
                    return;
                }
                await saveData();
                refreshUI();
            ...
            if (confirm(`¿Eliminar ${date}?`)) {
                try {
                    ...
                    if (confirm("¿Eliminar también las guardias de ese día?")) {
                        for (const guard of guards) {
                            if (guard.fecha === date) {
                                await fetch(API_URL + `guards.php?id=${guard.id}`, { method: 'DELETE' });
                            }
                        }
...
async function saveDayEdit() {
    ...
    try {
        ...
        await fetch(API_URL + `days.php?fecha=${currentEditingDay}`, { method: 'DELETE' });
        await fetch(API_URL + 'days.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    sorted.sort((a, b) => {
        ...
        switch (currentSortColumn) {
            case "date":
                ...
                break;
            case "worker":
                const wa = a.worker_id ? workers.find((w) => w.id === a.worker_id) : null;
                const wb = b.worker_id ? workers.find((w) => w.id === b.worker_id) : null;
                valA = wa ? wa.nombre : "";
                valB = wb ? wb.nombre : "";
                break;
            case "completed":
        ...
        if (guardsOfDay.length) {
            ...
            guardsOfDay.forEach((g) => {
                const worker = g.worker_id
                    ...
                    : null;
                const name = worker ? worker.nombre : g.worker_id ? "❌" : "Sin asignar";
...
function updateReportYearFilter() {
    ...
    if (!select) return;
    const years = [
        ...new Set(guards.map((g) => parseInt(g.fecha.split("-")[0])))
    ].sort((a, b) => a - b);
...
function bindEvents() {
    ...
    document.getElementById("exportDataBtn")?.addEventListener("click", downloadCurrentDataAsJson);
    document.getElementById("exportCsvBtn")?.addEventListener("click", exportToCSV);
    document.getElementById("importFileInput")?.addEventListener("change", (e) => {
        if (e.target.files[0]) importFromJSON(e.target.files[0]);
        e.target.value = "";
    });
    document.getElementById("resetAllChecksBtn")?.addEventListener("click", async () => {
        if (!isAdmin()) return alert("No tienes permisos.");
...
async function loadUsers() {
    try {
        ...
        USERS = [
            { id: 1, username: "admin", password: "admin", role: "admin", workerId: null },
            { id: 2, username: "miguel", password: "miguel", role: "worker", workerId: 1781469978315 },
        ];
